...
void setup() {
  ...
  /*
  int select = 0;
  if (select == 0) avatar.setFace(new AllFaces());
  else if (select == 1) avatar.setFace(new BaseFace());
  else if (select == 2) avatar.setFace(new AsciiFace());
  else if (select == 10) avatar.setFace(new LaputaFace());
  else avatar.setFace(new Face());
  */
  // M5Stack Avatar初期設定
...
void loop() {
  ...
  */
  ...
  //  barcode = 12345678910133;  // テスト用：スペシャル顔
